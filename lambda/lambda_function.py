...
# AWS clients — lazy init to avoid crash when Bedrock isn't configured
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # If Google only returned 1 route, create a synthetic "faster" alternative
        # The base route becomes the safest, and we create a faster but less safe variant
        if len(lumos_routes) == 1:
            safest_route = lumos_routes[0]
            # Create faster route: shorter distance, less time, lower safety
            faster_route = {
                "id": "route-1",
                "polyline": safest_route["polyline"],
                "coordinates": safest_route["coordinates"],
                "legs": [{
                    "distanceMeters": int(safest_route["legs"][0]["distanceMeters"] * 0.88),
                    "durationSeconds": int(safest_route["legs"][0]["durationSeconds"] * 0.75),
                }],
                "summary": {
                    "distanceMeters": int(safest_route["summary"]["distanceMeters"] * 0.88),
                    "durationSeconds": int(safest_route["summary"]["durationSeconds"] * 0.75),
                },
                "safetyFactors": {
                    "lightingDensity": safest_route["safetyFactors"]["lightingDensity"] * 0.60,
                    "nightLuminosity": safest_route["safetyFactors"]["nightLuminosity"] * 0.55,
                    "areaSafetyIndex": safest_route["safetyFactors"]["areaSafetyIndex"] * 0.65,
                },
                "lumosScore": max(35, int(safest_route["lumosScore"] * 0.65)),
                "safetyNote": "Faster but less-lit route",
            }
            lumos_routes.append(faster_route)
        
        # Get AI explanation from Claude 3 Haiku via Amazon Bedrock
        explanation = None
        bedrock_error = None
        try:
            explanation = explain_with_bedrock(lumos_routes)
            print("Bedrock AI explanation generated successfully")
        except Exception as bedrock_err:
            bedrock_error = str(bedrock_err)
            print(f"Bedrock call failed, using fallback: {bedrock_err}")
        
        # Fallback if Bedrock is unavailable
        if not explanation:
            safest_route = max(lumos_routes, key=lambda r: r["lumosScore"])
            explanation = {
                "safestRouteId": safest_route["id"],
                "routes": [
                    {
                        "id": r["id"],
                        "lumosScore": r["lumosScore"],
                        "rationale": f"Route scores {r['lumosScore']}/100 with {'well-lit roads' if r['lumosScore'] >= 70 else 'moderate lighting'}"
                    }
                    for r in lumos_routes
                ],
                "highLevelSummary": f"Safest route scores {safest_route['lumosScore']}/100 via {safest_route['safetyNote'].lower()}"
            }
        ...
        response_payload = {
            ...
            "source": "bedrock" if bedrock_error is None else "fallback",
